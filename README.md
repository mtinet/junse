...
### 3. Firebase 설정
1.  다운로드한 서비스 계정 키 파일의 이름을 `serviceAccountKey.json`으로 변경하여 프로젝트 루트 폴더에 넣습니다.
2.  `dashboard/firebase-config.js` 파일에 본인의 Firebase Web 설정 정보를 입력합니다.

### 4. 자동 실행 등록 (선택 사항)
컴퓨터가 켜질 때마다 자동으로 트래커가 실행되도록 설정하려면 포함된 PowerShell 스크립트를 사용하세요.
```powershell
# 관리자 권한으로 실행
